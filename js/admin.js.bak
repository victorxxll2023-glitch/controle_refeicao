...
(async () => {
  ...
  function confirmDelete(id, name) {
    ...
    // FIX: Clonar botão para remover todos os listeners anteriores
    // Isso resolve o bug de "não consigo excluir outro funcionário"
    const newBtn = confirmBtn.cloneNode(true);
    confirmBtn.parentNode.replaceChild(newBtn, confirmBtn);
    confirmBtn = newBtn; // atualiza referência local
    ...
    confirmBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        // FIX: Recarregar lista após exclusão
        await loadEmployees();
      } catch (err) {
  ...
  function renderSectorsPanel() {
    const panel = document.getElementById('tab-sectors');
    if (!panel) return;

    // Verificar se já renderizou o painel
    if (panel.querySelector('.sectors-container')) return;

    panel.innerHTML = `
      <div class="admin-toolbar no-print">
        <div>
          <h2>Setores / Unidades</h2>
          <p style="font-size:.82rem;color:var(--text-3)">Gerencie os setores do sistema</p>
        </div>
        <button class="btn btn-primary btn-lg" id="btn-add-sector">+ Novo Setor</button>
      </div>

      <div class="sectors-container table-wrap">
        <div class="table-toolbar">
          <div class="search-wrap" style="flex:1">
            <span class="search-icon">🔍</span>
            <input class="search-input" type="search" id="sector-search"
              placeholder="Buscar setor..." autocomplete="off">
          </div>
        </div>
        <div id="sectors-list" style="padding:8px 0">
    ...
    document.getElementById('sector-search')?.addEventListener('input',
      Utils.debounce(() => renderSectorsList(), 200)
    );
  ...
  function renderSectorsList() {
    ...
    const search = Utils.normalize(document.getElementById('sector-search')?.value || '');
    const list = search
    ...
    if (list.length === 0) {
      container.innerHTML = `
        <div class="empty-state" style="padding:32px">
          <span class="empty-icon">🏥</span>
          <p>${search ? 'Nenhum setor encontrado.' : 'Nenhum setor cadastrado.'}</p>
        </div>`;
      return;
    }
            ...
            <tr>
              <td>
                <span style="display:inline-flex;align-items:center;gap:8px">
                  <span style="width:10px;height:10px;border-radius:50%;background:${Utils.setorColor(s.name)};display:inline-block;flex-shrink:0"></span>
                  <strong>${Utils.escHtml(s.name)}</strong>
                </span>
              </td>
              <td>
                <div class="row-actions">
                  <button class="btn btn-ghost btn-sm" data-edit-sector="${s.id}" data-sector-name="${Utils.escHtml(s.name)}">✎ Editar</button>
